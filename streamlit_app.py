...
def load_model():
    ...
    if not os.path.exists(modelFile) or not os.path.exists(configFile):
        st.error(
            "Model files not found. Please make sure `deploy.prototxt` and `res10_300x300_ssd_iter_140000_fp16.caffemodel` are in the correct path.")
        return None
...
if img_file_buffer is not None:
    try:
        # Read the file and convert it to an OpenCV Image.
